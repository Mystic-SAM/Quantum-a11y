...
const setFocusToMenuitem= (newMenuitem): void => {
...
const setFocusToFirstMenuitem = (): void => {
    setFocusToMenuitem(firstMenuitem);
  }

const setFocusToLastMenuitem = (): void => {
    setFocusToMenuitem(lastMenuitem);
  }

const setFocusToPreviousMenuitem = (currentMenuitem: HTMLLIElement): HTMLLIElement => {
...
const setFocusToNextMenuitem = (currentMenuitem: HTMLLIElement): HTMLLIElement => {
    var newMenuitem, index;
...
const onMenuitemMouseover = (event): void => {
    let tgt = event.currentTarget;
    // console.log("tgtChild",tgt.children[0]);
    tgt.children[0].focus();
}

const onHamburgerItemBtnDown = (event): void => {
    let key = event.key;
    let tgt = event.currentTarget;
...
const toggleBtnInnerMenu = (target)=>{
    let listItem = target;
    const innerUlEl = listItem.children[1] as HTMLUListElement | null;
    if(innerUlEl) {
...
hamburgerLists.forEach( (listItem):void => {
...
const onBackgroundMousedown = (event): void => {
